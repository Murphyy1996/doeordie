...
namespace RootMotion.Demos
{
    ...
    public class SimpleAimingSystem : MonoBehaviour
    {
        ...
        private bool runOnceErrorFix = false;
        GameObject errorFixObj;
        ...
        void LimitAimTarget()
        {
            ...
            try
            {
                direction = (aim.solver.target.position - aimFrom);
            }
            catch
            {
                if (runOnceErrorFix == false)
                {
                    runOnceErrorFix = true;
                    GameObject foundParent = GameObject.Find("ErrorFixes");
                    if (foundParent == null)
                    {
                        foundParent = new GameObject();
                        foundParent.name = "ErrorFixes";
                    }
                    errorFixObj = new GameObject();
                    errorFixObj.name = "IK Error Fix";
                    errorFixObj.transform.SetParent(foundParent.transform);
                }
                aim.solver.target = errorFixObj.transform;
