...
public class CharacterControllerMovement : MonoBehaviour
{
	...
	private bool sprintToggleMode = false, isInSprint = false;
	...
	private void MovementCode() //This contains the movement code
	{
		...
		if (thisCC.isGrounded == false && gravityAllowed == true && forcedJumpPending == false)
		{
		...
		else //Do not apply gravity when grounded
		{
			...
			if (gravityAllowed == true)
			{
				...
				if (Input.GetKey(jumpKey) || Input.GetKey(jumpButtonController) && controllerEnabled == true || forcedJumpPending == true)
				{
	...
	{
		if (crouchScript != null && crouchScript.IsPlayerSliding() == false && grappleScript.IsCurrentlyGrappling() == true)
		{
